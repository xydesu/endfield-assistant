...
- **影拓豐碑** – 使用 `/monument` 以精美圖卡形式查看你的影拓豐碑通關進度（支援普通與苦難難度）
- **伺服器切換** – 使用 `/switch-server` 在綁定同個帳號的多個伺服器或角色之間進行切換
- **支援伺服器** – 使用 `/support` 取得官方 Discord 支援伺服器連結
...
| `/monument` | 查看影拓豐碑通關進度 | 所有人 |
| `/switch-server` | 切換至同帳號下的其他伺服器/角色 | 所有人 |
| `/stamina-notify <enable> [threshold] [tag]` | 設定體力滿載通知 | 所有人 |
| `/support` | 取得支援伺服器連結 | 所有人 |
